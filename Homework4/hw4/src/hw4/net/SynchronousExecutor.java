...
public class SynchronousExecutor {
    ...
    public SynchronousExecutor run() {
        ...
        for (int round = 0; round < network.getRounds()+1; round++) {
            //SEND PHASE OF CURRENT ROUND
            HashMapList<Node,Message> messages = new HashMapList();
            for (Node from : network.getNodes()) {
                for (Send send : from.safeSend(round)) {//send is key method in this phase
                    Node toNode = id2node.get(send.getTo());
                    if (toNode != null) {
                        if (network.hasEdge(from, toNode)) {
                            messages.put(toNode, new Message(from.getId(), send));
                        }
                    }
                }
            }
            ...
            //RECIEVE PHASE OF CURRENT ROUND
            for (Node n : network.getNodes()) {
                n.safeReceive(round, messages.get(n));//receive is key method in this phase
            }
        }
